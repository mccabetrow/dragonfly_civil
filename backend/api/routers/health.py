...
Security Posture:
- Error details are LOGGED but never exposed in responses
- Failed readiness returns generic "Upstream dependency unavailable"
...
# Root-level router for /health and /readyz at API root
root_router = APIRouter(tags=["Probes"])
...
class SimpleReadinessResponse(BaseModel):
    """Readiness response with services status."""
...
@root_router.get(
    "/readyz",
    response_model=SimpleReadinessResponse,
    responses={
        200: {"description": "Service is ready to accept traffic"},
        503: {
            "description": "Service is not ready",
            "model": SimpleReadinessErrorResponse,
        },
    },
...
async def root_readiness_check() -> JSONResponse:
    ...
    try:
        ...
        # Attempt SELECT 1 with strict timeout
