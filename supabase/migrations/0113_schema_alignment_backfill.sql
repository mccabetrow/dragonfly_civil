-- 0113_schema_alignment_backfill.sql
-- Ensure existing environments gain the plaintiff schema columns, views, and RPCs enforced by tooling.
-- migrate:up
BEGIN;
-- Plaintiffs + intake tables
...
GRANT SELECT ON public.v_plaintiff_call_queue TO anon,
    authenticated,
    service_role;
-- Ensure PostgREST sees the refreshed RPC/View definitions.
DO $$ BEGIN PERFORM public.pgrst_reload();
EXCEPTION
WHEN undefined_function THEN RAISE NOTICE 'public.pgrst_reload() unavailable; skipping cache refresh.';
END $$;
COMMIT;
-- migrate:down
BEGIN;
