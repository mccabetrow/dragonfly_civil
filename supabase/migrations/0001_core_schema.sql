...
-- Enumerated types
create type judgments.case_status as enum ('new','enriched','contacting','intake','enforcing','collected','dead');
create type parties.entity_type as enum ('person','company');
create type enrichment.contact_kind as enum ('phone','email','address');
create type enforcement.action_type as enum ('levy','income_exec','lien','turnover');
...
create table parties.entities (
  ...
  name_norm text generated always as (regexp_replace(lower(name_raw), '\s+', ' ', 'g')) stored,
...
create table enrichment.assets (
  ...
  entity_id uuid not null references parties.entities(entity_id) on delete cascade,
  asset_type text check (asset_type in ('real_property','bank_hint','employment','vehicle','license','ucc','dba')),
...
create table enrichment.collectability (
  ...
  total_score numeric(5,2) generated always as (
    greatest(0, identity_score*0.30 + contactability_score*0.25 + asset_score*0.25 + recency_amount_score*0.10 - adverse_penalty)
  ) stored,
  tier text generated always as (
    case
      when greatest(0, identity_score*0.30 + contactability_score*0.25 + asset_score*0.25 + recency_amount_score*0.10 - adverse_penalty) >= 80 then 'A'
      when greatest(0, identity_score*0.30 + contactability_score*0.25 + asset_score*0.25 + recency_amount_score*0.10 - adverse_penalty) >= 60 then 'B'
      when greatest(0, identity_score*0.30 + contactability_score*0.25 + asset_score*0.25 + recency_amount_score*0.10 - adverse_penalty) >= 40 then 'C'
      else 'D'
    end
  ) stored,
...
create table outreach.attempts (
  ...
  case_id uuid not null references judgments.cases(case_id) on delete cascade,
  cadence_id uuid references outreach.cadences(cadence_id) on delete set null,
...
create index idx_attempts_case_time on outreach.attempts (case_id, attempted_at);
create index idx_entities_name_norm_trgm on parties.entities using gin (name_norm gin_trgm_ops);
...
create policy select_collectability on enrichment.collectability for select using (true);
...
create policy insert_cases_service on judgments.cases for insert with check (auth.role() = 'service_role');
create policy insert_entities_service on parties.entities for insert with check (auth.role() = 'service_role');
create policy insert_roles_service on parties.roles for insert with check (auth.role() = 'service_role');
create policy insert_contacts_service on enrichment.contacts for insert with check (auth.role() = 'service_role');
create policy insert_assets_service on enrichment.assets for insert with check (auth.role() = 'service_role');
create policy insert_collectability_service on enrichment.collectability for insert with check (auth.role() = 'service_role');
create policy insert_cadences_service on outreach.cadences for insert with check (auth.role() = 'service_role');
create policy insert_attempts_service on outreach.attempts for insert with check (auth.role() = 'service_role');
create policy insert_esign_service on intake.esign for insert with check (auth.role() = 'service_role');
create policy insert_actions_service on enforcement.actions for insert with check (auth.role() = 'service_role');
create policy insert_trust_txns_service on finance.trust_txns for insert with check (auth.role() = 'service_role');
create policy insert_runs_service on ops.runs for insert with check (auth.role() = 'service_role');

-- Update policies (service role only)
create policy update_cases_service on judgments.cases for update using (auth.role() = 'service_role') with check (auth.role() = 'service_role');
create policy update_entities_service on parties.entities for update using (auth.role() = 'service_role') with check (auth.role() = 'service_role');
create policy update_roles_service on parties.roles for update using (auth.role() = 'service_role') with check (auth.role() = 'service_role');
create policy update_contacts_service on enrichment.contacts for update using (auth.role() = 'service_role') with check (auth.role() = 'service_role');
create policy update_assets_service on enrichment.assets for update using (auth.role() = 'service_role') with check (auth.role() = 'service_role');
create policy update_collectability_service on enrichment.collectability for update using (auth.role() = 'service_role') with check (auth.role() = 'service_role');
create policy update_cadences_service on outreach.cadences for update using (auth.role() = 'service_role') with check (auth.role() = 'service_role');
create policy update_attempts_service on outreach.attempts for update using (auth.role() = 'service_role') with check (auth.role() = 'service_role');
create policy update_esign_service on intake.esign for update using (auth.role() = 'service_role') with check (auth.role() = 'service_role');
create policy update_actions_service on enforcement.actions for update using (auth.role() = 'service_role') with check (auth.role() = 'service_role');
create policy update_trust_txns_service on finance.trust_txns for update using (auth.role() = 'service_role') with check (auth.role() = 'service_role');
create policy update_runs_service on ops.runs for update using (auth.role() = 'service_role') with check (auth.role() = 'service_role');
...
left join enrichment.collectability co on co.case_id = c.case_id;
