-- 0121_case_copilot_priority_pipeline_repair2.sql
-- Repairs case copilot + priority pipeline views/RPCs after 0120 applied without restoring them.
-- Recreates: v_case_copilot_latest, v_pipeline_snapshot, v_priority_pipeline,
-- request_case_copilot(uuid, text), log_call_outcome(uuid, uuid, text, text, text, timestamptz).
BEGIN;
...
GRANT SELECT ON public.v_case_copilot_latest TO anon,
    authenticated,
    service_role;
-- RPC to re-queue Case Copilot summaries.
...
GRANT EXECUTE ON FUNCTION public.request_case_copilot(uuid, text) TO anon,
    authenticated,
    service_role;
-- Six-argument log_call_outcome RPC for outreach workflows.
...
GRANT EXECUTE ON FUNCTION public.log_call_outcome(uuid, uuid, text, text, text, timestamptz) TO anon,
    authenticated,
    service_role;
-- Pipeline snapshot view used by dashboards + tooling.
...
GRANT SELECT ON public.v_pipeline_snapshot TO anon,
    authenticated,
    service_role;
-- Priority pipeline ranking view.
