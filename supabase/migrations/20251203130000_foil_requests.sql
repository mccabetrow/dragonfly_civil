...
CREATE POLICY foil_requests_service_all ON public.foil_requests FOR ALL USING (auth.role() = 'service_role');
CREATE POLICY foil_followup_log_service_all ON public.foil_followup_log FOR ALL USING (auth.role() = 'service_role');
-- Allow authenticated users to read
CREATE POLICY foil_requests_select ON public.foil_requests FOR
SELECT USING (auth.role() IN ('authenticated', 'anon'));
CREATE POLICY foil_followup_log_select ON public.foil_followup_log FOR
SELECT USING (auth.role() IN ('authenticated', 'anon'));
-- Grants
