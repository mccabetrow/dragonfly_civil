-- =============================================================================
-- Migration: Reaper Heartbeat & Hardening
-- =============================================================================
...
--   ops.reap_stuck_jobs() - Updated to write heartbeat
--
-- Views:
--   ops.v_reaper_status - Real-time reaper health for monitoring
--
...
END;
$$;
COMMENT ON FUNCTION ops.reap_stuck_jobs(INTERVAL) IS 'Reaps stuck jobs and records heartbeat. Monitored by watchdog for staleness.';
-- -----------------------------------------------------------------------------
...
GRANT EXECUTE ON FUNCTION ops.record_reaper_heartbeat(INT, TEXT, TEXT) TO postgres;
-- Grant to the specific overload of reap_stuck_jobs
DO $$ BEGIN -- Try to grant to the overload with integer parameter
IF EXISTS (
    SELECT 1
    FROM pg_proc p
...
) THEN EXECUTE 'GRANT EXECUTE ON FUNCTION ops.reap_stuck_jobs(integer) TO postgres';
END IF;
EXCEPTION
WHEN undefined_function THEN RAISE NOTICE 'ops.reap_stuck_jobs(integer) not found, skipping grant';
END $$;
COMMIT;